export const navLinks = [
  { name: '소개', href: '#about' },
  { name: '교육자료', href: '#edu-archive' },
  { name: '수상내역', href: '#hall-of-fame' },
  { name: '영상', href: '#media-room' },
  { name: '웹앱/보도', href: '#web-app-lab' },
  { name: '저서', href: '#publications' },
];

export const skillsData = [
  {
    title: 'Top-tier Educator',
    description: '수업혁신사례연구대회 전국 2등급(교육부장관표창) 및 전남교육청 우수강사, 수많은 컨설팅과 연수를 진행하는 현직 초등 교사입니다.',
    icon: 'GraduationCap',
  },
  {
    title: 'Edutech & AI Leader',
    description: 'AI 중점학교 및 디지털 선도학교 주무를 맡고 있으며, AIDT 강사 및 에듀테크 현장 지원단으로 미래 교육을 이끌고 있습니다.',
    icon: 'Laptop',
  },
  {
    title: 'Author & Creator',
    description: '독서미션으로 끝장내기 외 9권의 책을 집필한 작가이며, 전남초등아카펠라연구회 보컬퍼커션으로도 활동하는 크리에이터입니다.',
    icon: 'PenTool',
  },
];

export const eduResourcesData = [
  {
    id: 1,
    title: '인디스쿨 엽쌤스쿨 교육자료',
    description: '전국 초등교사 커뮤니티 인디스쿨에 공유된 엽쌤의 다양하고 혁신적인 수업 자료들을 확인해보세요.',
    thumbnail: 'https://images.unsplash.com/photo-1503676260728-1c00da094a0b?auto=format&fit=crop&q=80&w=800',
    tags: ['#인디스쿨', '#수업자료', '#초등교육'],
    link: 'https://indischool.com/@user359088',
  },
  {
    id: 2,
    title: '대한민국 글로컬 박람회 미래 교실',
    description: '2024년 대한민국 글로컬 박람회에서 선보인 국어과 대표 수업 및 미래 교실 모델의 자료입니다.',
    thumbnail: 'https://images.unsplash.com/photo-1516321497487-e288fb19713f?auto=format&fit=crop&q=80&w=800',
    tags: ['#미래교실', '#글로컬박람회', '#국어'],
    link: '#',
  },
  {
    id: 3,
    title: '독서인문선도교실 실천 사례',
    description: '학생들의 문해력과 인문학적 소양을 기르기 위한 독서인문선도교실 공개수업 및 운영 자료입니다.',
    thumbnail: 'https://images.unsplash.com/photo-1456513080510-7bf3a84b82f8?auto=format&fit=crop&q=80&w=800',
    tags: ['#독서인문', '#문해력', '#연구학교'],
    link: '#',
  },
  {
    id: 4,
    title: '발명영재교육센터 발명 실습',
    description: '2022년부터 진행해 온 인문발명 및 창의융합 발명영재교육센터의 교육 커리큘럼 및 실습 자료.',
    thumbnail: 'https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?auto=format&fit=crop&q=80&w=800',
    tags: ['#발명교육', '#영재교육', '#창의융합'],
    link: '#',
  },
];

export const awardsData = [
  {
    id: 1,
    title: '2024 수업혁신사례연구대회 전국 2등급 (부총리 겸 교육부장관표창)',
    date: '2024',
    image: 'https://images.unsplash.com/photo-1523050854058-8df90110c9f1?auto=format&fit=crop&q=80&w=800',
  },
  {
    id: 2,
    title: '2024 대한민국 글로컬 박람회 유공교원 (교육감표창)',
    date: '2024',
    image: 'https://images.unsplash.com/photo-1596495578065-6e0763fa1178?auto=format&fit=crop&q=80&w=800',
  },
  {
    id: 3,
    title: '2024 과학교육 및 자연관찰탐구방법개선 연구대회 3등급 (교육감표창)',
    date: '2024',
    image: 'https://images.unsplash.com/photo-1532094349884-543bc11b234d?auto=format&fit=crop&q=80&w=800',
  },
  {
    id: 4,
    title: '2023 독서인문실천사례 연구대회 2등급 (교육감표창)',
    date: '2023',
    image: 'https://images.unsplash.com/photo-1513475382585-d06e58bcb0e0?auto=format&fit=crop&q=80&w=800',
  },
  {
    id: 5,
    title: '(21~23) Yes24 어린이 독후감 대회 지도교사상 및 최우수 (전국 2위)',
    date: '2021-2023',
    image: 'https://images.unsplash.com/photo-1491841550275-ad7854e35ca6?auto=format&fit=crop&q=80&w=800',
  },
  {
    id: 6,
    title: '(22~24) 발명 교육 유공 교원 표창 (교육감표창)',
    date: '2022-2024',
    image: 'https://images.unsplash.com/photo-1507676184212-d0c30a3c37ce?auto=format&fit=crop&q=80&w=800',
  },
  {
    id: 7,
    title: '제23회 불조심 어린이 마당 전남 1위, 전국 7위 (교육감표창)',
    date: '2023',
    image: 'https://images.unsplash.com/photo-1565843708714-52ecf69ab81f?auto=format&fit=crop&q=80&w=800',
  },
  {
    id: 8,
    title: '제20회 전국어린이통계활용대회 은상 (통계청장상)',
    date: '2022',
    image: 'https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&q=80&w=800',
  },
];

export const mediaData = [
  {
    id: 1,
    title: '엽쌤스쿨 교육 영상 및 노하우',
    thumbnail: 'https://img.youtube.com/vi/h0qWm-Ucwgs/hqdefault.jpg',
    url: 'https://www.youtube.com/watch?v=h0qWm-Ucwgs',
    duration: 'YouTube',
  },
  {
    id: 2,
    title: '학생들과 함께하는 창의적 교실 이야기',
    thumbnail: 'https://img.youtube.com/vi/6cLMzs7Evk4/hqdefault.jpg',
    url: 'https://www.youtube.com/watch?v=6cLMzs7Evk4',
    duration: 'YouTube',
  },
  {
    id: 3,
    title: '아카펠라 아카라카 공연 영상',
    thumbnail: 'https://img.youtube.com/vi/Lx2b_nWgYSg/hqdefault.jpg',
    url: 'https://www.youtube.com/watch?v=Lx2b_nWgYSg&t=15s',
    duration: 'YouTube',
  },
  {
    id: 4,
    title: '디지털 미래 교실 수업 현장',
    thumbnail: 'https://img.youtube.com/vi/LMKwGQjb5D4/hqdefault.jpg',
    url: 'https://www.youtube.com/watch?v=LMKwGQjb5D4',
    duration: 'YouTube',
  },
  {
    id: 5,
    title: '발명영재 및 창의 챔피언 대회 지도',
    thumbnail: 'https://img.youtube.com/vi/iLN9jfrhii4/hqdefault.jpg',
    url: 'https://www.youtube.com/watch?v=iLN9jfrhii4',
    duration: 'YouTube',
  },
  {
    id: 6,
    title: '초등 독서 인문 교육의 실제',
    thumbnail: 'https://img.youtube.com/vi/OqUSvovKbaw/hqdefault.jpg',
    url: 'https://www.youtube.com/watch?v=OqUSvovKbaw',
    duration: 'YouTube',
  },
];

export const webAppsData = [
  {
    id: 1,
    title: 'AI 중점 및 디지털 선도학교 프로젝트',
    description: '2026년 여수한려초등학교 AI 중점학교 및 디지털 활용 선도학교 주무로서, 다양한 에듀테크 서비스와 AI 기술을 실제 교실에 도입하고 적용하는 연구를 진행합니다. AIDT 강사 및 에듀테크 현장 지원단으로 활동하며 교사들의 디지털 역량 강화를 돕습니다.',
    image: 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?auto=format&fit=crop&q=80&w=1200',
    techStack: ['AI 에듀테크', '디지털 선도학교', 'AIDT 강사'],
    link: '#',
  },
  {
    id: 2,
    title: '[언론보도] 예스24, 어린이 독후감 대회 지도교사상 수상',
    description: '학생 저자되기 프로젝트 및 나도 작가 프로젝트를 성공적으로 이끌며, (21~23년) YES24 어린이 독후감 대회에서 지도교사상 및 최우수상을 배출한 성과가 여러 언론을 통해 보도되었습니다.',
    image: 'https://images.unsplash.com/photo-1585829365295-ab7cd400c167?auto=format&fit=crop&q=80&w=1200',
    techStack: ['YES24', '학생저자프로젝트', '독서인문선도'],
    link: 'https://ch.yes24.com/Article/Details/51895',
  },
  {
    id: 3,
    title: '[언론보도] 전남학생창의력챔피언대회 빛낸 선생님들',
    description: '여수발명교육센터 영재 강사로서 전남창의융합인재양성프로젝트 성과를 공유하고, 대한민국 창의력 챔피언 대회를 지도하여 우수한 성적을 거두었으며, 아카펠라 그룹 아카라카로 85회 이상 공연한 이력이 보도되었습니다.',
    image: 'https://images.unsplash.com/photo-1511629091441-ee46146481b6?auto=format&fit=crop&q=80&w=1200',
    techStack: ['창의융합인재', '발명영재교육', '아카라카 공연'],
    link: 'http://yeosuinfo92.netproserv.com/bbs/board.php?bo_table=news&wr_id=26801',
  },
];

export const publicationsData = [
  {
    id: 1,
    title: '선생님 교실 인문(독서논술) 수업을 훔쳐내다',
    description: '여수(미)사랑, 우리들의 이야기 2 외 다수 집필 (18,900원)',
    cover: 'https://image.yes24.com/goods/152574991/XL',
    link: 'https://www.yes24.com/product/goods/152574991',
  },
  {
    id: 2,
    title: '여수(美)사랑, 우리들의 이야기',
    description: '나, 그리고 우리들의 이야기 외 공저 (14,000원)',
    cover: 'https://image.yes24.com/goods/152467965/XL',
    link: 'https://www.yes24.com/product/goods/152467965',
  },
  {
    id: 3,
    title: '우리들의 뇌물 상자',
    description: '예스24 추천 저서 (16,000원)',
    cover: 'https://image.yes24.com/goods/147796751/XL',
    link: 'https://www.yes24.com/product/goods/147796751',
  },
  {
    id: 4,
    title: '아무도 모르는 5학년의 속마음',
    description: '예스24 추천 저서 (11,900원)',
    cover: 'https://image.yes24.com/goods/134023092/XL',
    link: 'https://www.yes24.com/product/goods/134023092',
  },
  {
    id: 5,
    title: '해를 닮은 아이',
    description: '예스24 추천 저서 (11,000원)',
    cover: 'https://image.yes24.com/goods/140283591/XL',
    link: 'https://www.yes24.com/product/goods/140283591',
  },
  {
    id: 6,
    title: 'AI와 킹콩 그림 꿈의 조각',
    description: '예스24 추천 저서 (11,000원)',
    cover: 'https://image.yes24.com/goods/138744485/XL',
    link: 'https://www.yes24.com/product/goods/138744485',
  },
  {
    id: 7,
    title: '안심하고 읽는 94가지 이야기',
    description: '예스24 추천 저서 (17,000원)',
    cover: 'https://image.yes24.com/goods/123253561/XL',
    link: 'https://www.yes24.com/product/goods/123253561',
  },
  {
    id: 8,
    title: '나, 그리고 우리들의 이야기',
    description: '엽쌤외 | BOOKK(부크크) (12,000원)',
    cover: 'https://image.yes24.com/goods/165140071/XL',
    link: 'https://www.yes24.com/product/goods/165140071',
  },
  {
    id: 9,
    title: '작가작가, 우리들의 이야기',
    description: '문예노트 외 19명 공저 (13,000원)',
    cover: 'https://file.bookk.co.kr/books/20250930/68da3d4ff3bea5e94f39faa3/thumbnails/1759189304881.jpg',
    link: 'https://bookk.co.kr/bookStore/68da404cee308e902a61b900',
  }
];
